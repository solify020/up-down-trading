...
const Balance = () => {
    ...
    const handleDeposit = () => {
        axios.post('/api/confirm_deposit', { amount: amount }, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
            }
        }).then((res) => {
            if (res.data.message == 'Deposit confirmed') {
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#2a0845] to-[#25003e] flex flex-col gap-8 items-center justify-center py-8 px-2">
            <div className="w-full max-w-md bg-[#2d1457] rounded-2xl shadow-2xl px-8 py-6 border border-white/10">
                ...
                <div className="mb-10">
                    ...
                    <label className="block text-base font-semibold mb-2 text-white/90 mt-8">Your Balance</label>
                    <div className="flex flex-row gap-3">
                        <input
                            type="text"
                            value={balance}
